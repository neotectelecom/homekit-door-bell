 ...
 * motion sensor using a Push Button for Door Bell
 * This uses an OTA mechanism created by HomeACcessoryKid
 *
 */

#define DEVICE_MANUFACTURER "Neotec"
#define DEVICE_NAME "Door-Bell"
#define DEVICE_MODEL "ESP-01S"
#define DEVICE_SERIAL "21470083"
...
void motion_sensor_callback(uint8_t gpio) {
    ...
    if (gpio == MOTION_SENSOR_GPIO){
        int timbre = 0;
        timbre = gpio_read(MOTION_SENSOR_GPIO);
        motion_detected.value = HOMEKIT_BOOL(timbre);
        homekit_characteristic_notify(&motion_detected, HOMEKIT_BOOL(timbre));
        if (timbre == 1) {
		printf("Sonando el Timbre en %d\n", gpio);
